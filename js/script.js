/* ==========================================================================
   ...
   Hooks used here match index.html / css/style.css:
...
/* ==========================================================================
   PENTAGAN — ADD-ON SCRIPTS
   Append this to the END of js/script.js (it is its own IIFE, so it
   won't collide with anything already in that file).
...
(function () {
  ...
  /* ---------------------------------------------------------------
     ...
     server endpoint or a service like Formspree, replace sendWhatsApp
...
/* ==========================================================================
   PENTAGAN — TESTIMONIALS SLIDER JS
   Append this to the end of js/script.js (own IIFE, no collisions).
   Works with testimonials-slider.html + testimonials-slider.css.
   ========================================================================== */
(function () {
  'use strict';
  ...
  var AUTOPLAY_DELAY = 4500;      // ms between auto-advances
  var prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  /* ---------------------------------------------------------------
     How many cards are visible at once, and how far one "page" is.
     Read from the actual rendered card width so this stays correct
     across the 3 / 2 / 1-per-view breakpoints without duplicating
     the CSS numbers here.
  ---------------------------------------------------------------- */
  function cardStep() {
  ...
  /* ---------------------------------------------------------------
     Dots — one per "page" of the slider, not one per card
  ---------------------------------------------------------------- */
  function buildDots() {
    if (!dotsWrap) return;
    dotsWrap.innerHTML = '';
  ...
  /* ---------------------------------------------------------------
     Keep index correct if the visitor scrolls/swipes by hand,
     without fighting their touch gesture.
  ---------------------------------------------------------------- */
  var scrollEndTimer = null;
  track.addEventListener('scroll', function () {
    clearTimeout(scrollEndTimer);
    scrollEndTimer = setTimeout(function () {
      currentIndex = Math.max(0, Math.min(indexFromScroll(), maxIndex()));
      syncArrows();
      syncDots();
    }, 120);
  }, { passive: true });
  ...
  /* ---------------------------------------------------------------
     Keyboard support when the viewport itself is focused
  ---------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------
     Autoplay — pauses on hover, touch, or keyboard focus so people
     reading a quote aren't interrupted mid-sentence.
  ---------------------------------------------------------------- */
  function startAutoplay() {
    if (prefersReducedMotion || AUTOPLAY_DELAY <= 0) return;
    stopAutoplay();
  ...
  ['mouseenter', 'focusin', 'touchstart'].forEach(function (evt) {
    viewport.addEventListener(evt, stopAutoplay, { passive: true });
  });
  ['mouseleave', 'focusout'].forEach(function (evt) {
    viewport.addEventListener(evt, startAutoplay);
  });

  // pause entirely when the section scrolls off-screen
  if ('IntersectionObserver' in window) {
    new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
  ...
  /* ---------------------------------------------------------------
     Rebuild dots / clamp position on resize (breakpoint changes
     the number of visible cards, so page count changes too)
  ---------------------------------------------------------------- */
